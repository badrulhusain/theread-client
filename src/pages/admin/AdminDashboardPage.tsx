  ...
  return (
    <div className="space-y-6 p-4 md:p-8">
      <div>
        ...
        <h2 className="mt-1 font-serif text-4xl font-semibold tracking-tight text-[#231b17]">Admin dashboard</h2>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <Stat label="Submitted" value={count(stats.submittedBlogs)} loading={loading} />
        <Stat label="Under review" value={count(stats.underReviewBlogs)} loading={loading} />
        <Stat label="Approved" value={count(stats.approvedBlogs)} loading={loading} />
        <Stat label="Published" value={count(stats.publishedBlogs)} loading={loading} />
        <Stat label="Rejected" value={count(stats.rejectedBlogs)} loading={loading} />
      </div>
      <Card><CardContent className="flex flex-wrap gap-2"><Button asChild><Link to="/admin/users">Manage users</Link></Button><Button asChild variant="outline"><Link to="/admin/blogs">Manage blogs</Link></Button><Button asChild variant="outline"><Link to="/admin/blogs?status=APPROVED">Ready to publish</Link></Button></CardContent></Card>
      <section>
        <h3 className="mb-3 font-serif text-2xl font-semibold text-[#231b17]">Recent admin items</h3>
        {loading ? <div className="grid gap-4">{[1, 2].map((item) => <div key={item} className="h-44 animate-pulse rounded-2xl border border-[#ded3c4] bg-[#fbf7ef]" />)}</div> : blogs.length ? (
          <div className="grid gap-4 md:grid-cols-2">{blogs.map((blog) => <BlogCard key={blog.id} blog={blog} showStatus />)}</div>
        ) : (
          <Card><CardContent className="text-[#74685f]">No recent admin items yet.</CardContent></Card>
        )}
      </section>
    </div>
  );
